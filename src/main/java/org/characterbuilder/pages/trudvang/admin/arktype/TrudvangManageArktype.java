...
/**
 * TrudvangManageArktype
 *
 * @author <a href="mailto:jens.brimberg@gmail.com">Jens Brimberg</a>
 */
public class TrudvangManageArktype extends TrudvangManage {

	private JPAContainer<TrudvangArktype> container = null;
	private MyHorizontalLayout mainLayout = new MyHorizontalLayout();
	private MyVerticalLayout editArea = new MyVerticalLayout();

	public TrudvangManageArktype() {
		container = JPAContainerFactory.make(TrudvangArktype.class, ThePersister.ENTITY_MANAGER_NAME);
	...
	private class TableListener implements Property.ValueChangeListener {
		...
		public void valueChange(Property.ValueChangeEvent event) {
			...
			TrudvangArktype selectedExcep = getSelectedArktype();
	...
	public TrudvangArktype getSelectedArktype() {
		try {
	...
	private class ExceptionalPanel extends Panel {
		...
		public ExceptionalPanel(TrudvangArktype arktype) {
			super();
			buildUI();
			
			nameField.setValue(arktype.getName());
			descriptionArea.setValue(arktype.getDescription());
			bookSelect.select(arktype.getBookId());
		...
		private class AddListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					TrudvangArktype newExceptional = new TrudvangArktype();
		...
		private class EditListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					TrudvangArktype edit = getSelectedArktype();
					if (edit != null) {
						EntityManager em = ThePersister.getEntityManager();
						em.getTransaction().begin();
						edit = em.find(TrudvangArktype.class,
								  edit.getId());
		...
		private class RemoveListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					TrudvangArktype rem = getSelectedArktype();
					if (rem != null) {
						EntityManager em = ThePersister.getEntityManager();
						em.getTransaction().begin();
						em.remove(rem);
