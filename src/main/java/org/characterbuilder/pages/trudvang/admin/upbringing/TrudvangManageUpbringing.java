...
/**
 * TrudvangManageUpbringing
 * @author <a href="mailto:jens.brimberg@gmail.com">Jens Brimberg</a>
 */
public class TrudvangManageUpbringing extends TrudvangManage{
	
	private JPAContainer<TrudvangUpbringing> container = null;
	private MyHorizontalLayout mainLayout = new MyHorizontalLayout();
	private MyVerticalLayout editArea = new MyVerticalLayout();

	public TrudvangManageUpbringing() {
		container = JPAContainerFactory.make(TrudvangUpbringing.class, ThePersister.ENTITY_MANAGER_NAME);
	...
	private class TableListener implements Property.ValueChangeListener {
		...
		public void valueChange(Property.ValueChangeEvent event) {
			...
			TrudvangUpbringing selectedExcep = getSelectedUpbringing();
	...
	public TrudvangUpbringing getSelectedUpbringing() {
		try {
	...
	private class ExceptionalPanel extends Panel {
		...
		public ExceptionalPanel(TrudvangUpbringing excep) {
			super();
			buildUI();
		...
		private class AddListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					TrudvangUpbringing newExceptional = new TrudvangUpbringing();
		...
		private class EditListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					TrudvangUpbringing edit = getSelectedUpbringing();
					if (edit != null) {
						EntityManager em = ThePersister.getEntityManager();
						em.getTransaction().begin();
						edit = em.find(TrudvangUpbringing.class,
								  edit.getId());
		...
		private class RemoveListener implements Button.ClickListener {
			...
			public void buttonClick(ClickEvent event) {
				try {
					TrudvangUpbringing rem = getSelectedUpbringing();
					if (rem != null) {
						EntityManager em = ThePersister.getEntityManager();
						em.getTransaction().begin();
						em.remove(rem);
